...
function DiscoveryCard({ item }: { item: DiscoverySection["items"][number] }) {
  ...
  return (
    <Link href={href} className="block rounded-3xl border border-[var(--BeaconVie-border)] bg-[var(--BeaconVie-card)] p-5 shadow-sm transition hover:border-violet-300 hover:bg-[var(--BeaconVie-card-soft)] dark:hover:border-violet-400/40">
      <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-100 text-violet-700">
        <Sparkles size={20} />
      </div>
      <h3 className="line-clamp-2 text-lg font-black text-[var(--BeaconVie-ink)]">{item.title}</h3>
      <p className="mt-2 line-clamp-2 text-sm font-bold leading-6 text-[var(--BeaconVie-muted)]">
      ...
      <div className="mt-4 inline-flex items-center gap-2 text-sm font-black text-violet-700">
        {isRecommendation(item) ? item.ctaLabel : "Mở"} <ArrowRight size={16} />
      </div>
    </Link>
  );
}
    ...
    <div className="space-y-6 pb-10">
      ...
      {loading ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {Array.from({ length: 8 }).map((_, index) => <div key={index} className="h-48 animate-pulse rounded-3xl bg-[var(--BeaconVie-card-soft)]" />)}
        </div>
      ) : error ? (
        ...
        sections.map((section) => (
          <section key={section.id} className="space-y-4">
            <div>
              <h2 className="text-2xl font-black text-[var(--BeaconVie-ink)]">{section.title}</h2>
              <p className="text-sm font-bold text-[var(--BeaconVie-muted)]">{section.description}</p>
            </div>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <section className="rounded-3xl border border-dashed border-[var(--BeaconVie-border)] bg-[var(--BeaconVie-card-soft)] p-8 text-center">
          <p className="font-black text-[var(--BeaconVie-ink)]">No discovery content yet.</p>
          <p className="mt-2 text-sm font-bold text-[var(--BeaconVie-muted)]">Complete a lesson or placement test to improve recommendations.</p>
        </section>
      )}
    </div>
  );
