  ...
  if (loading) {
    return (
      <div className="space-y-5">
        <div className="h-36 animate-pulse rounded-3xl bg-[var(--BeaconVie-card-soft)]" />
        <div className="grid gap-4 md:grid-cols-3">
          {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className="h-28 animate-pulse rounded-3xl bg-[var(--BeaconVie-card-soft)]" />
          ))}
        </div>
        <div className="h-80 animate-pulse rounded-3xl bg-[var(--BeaconVie-card-soft)]" />
      </div>
    );
  }
      ...
      <section className="grid gap-4 md:grid-cols-4">
        {[
          ...
          <div key={label} className="rounded-3xl border border-[var(--BeaconVie-border)] bg-[var(--BeaconVie-card)] p-5 shadow-sm">
            ...
            <p className="text-sm font-bold text-[var(--BeaconVie-muted)]">{label}</p>
          </div>
        ))}
      </section>
          ...
          <div className="grid h-64 grid-cols-7 items-end gap-2 overflow-x-auto sm:gap-3">
            {data.activityTrend.map((item) => (
              <div key={item.date} className="flex h-full min-w-10 flex-col justify-end gap-2">
                <div className="flex flex-1 items-end rounded-2xl bg-[var(--BeaconVie-card-soft)] px-2 pb-2">
                  <div
                ...
                <p className="text-center text-[11px] font-bold text-[var(--BeaconVie-muted)]">
                  {item.date.slice(5)}
                </p>
              </div>
            ))}
          ...
          <section className="rounded-3xl border border-violet-100 bg-violet-50 p-5">
            ...
            <div className="mt-4 space-y-2">
              {data.aiReport.insights.map((item) => (
                <p key={item} className="rounded-2xl border border-[var(--BeaconVie-border)] bg-[var(--BeaconVie-card)] p-3 text-sm font-bold text-[var(--BeaconVie-muted)]">
                  {item}
                </p>
              ))}
            </div>
              ...
              {data.recentActivities.map((activity) => (
                <Link key={activity.id} href={activity.actionUrl} className="flex items-center gap-3 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-black text-[var(--BeaconVie-ink)]">{activity.title}</p>
                    <p className="truncate text-sm font-bold text-[var(--BeaconVie-muted)]">{activity.description}</p>
                  </div>
    ...
    <section className="grid gap-6 lg:grid-cols-2">
      <div className="rounded-3xl border border-[var(--BeaconVie-border)] bg-[var(--BeaconVie-card)] p-5 shadow-sm">
        ...
        {weaknessError ? (
          ...
          <div className="space-y-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="h-14 animate-pulse rounded-2xl bg-[var(--BeaconVie-card-soft)]" />
            ))}
          </div>
          ...
          <div className="space-y-3">
            {weaknesses.overallWeakest.map((item) => (
              <Link
                ...
                className="block rounded-2xl border border-[var(--BeaconVie-border)] p-4 hover:bg-[var(--BeaconVie-card-soft)]"
              >
      ...
      <div className="rounded-3xl border border-violet-100 bg-violet-50 p-5">
        ...
        {coachError ? (
          ...
          <>
            <h2 className="mt-3 text-xl font-black text-[var(--BeaconVie-ink)]">{coach.headline}</h2>
            {coach.whyThisLesson ? (
            ...
            {coach.weeklyPlan.length > 0 ? (
              <div className="mt-4 space-y-2">
                {coach.weeklyPlan.map((item) => (
                  <p
                    key={item}
                    className="rounded-2xl border border-[var(--BeaconVie-border)] bg-[var(--BeaconVie-card)] p-3 text-sm font-bold text-[var(--BeaconVie-muted)]"
                  >
                    {item}
                  </p>
                ))}
