...
- **Frontend**: new screens under `english-web-build/src/Components/Arena/`, reusing existing BeaconVie CSS custom properties already in use (`--beaconvie-primary`, `--beaconvie-ink`, `--beaconvie-card`, `--beaconvie-muted`, `--beaconvie-violet`, `--beaconvie-border` â€” confirmed present in `ArenaPage.tsx`/`ArenaRoomPage.tsx` this session) and the existing `ArenaModal` component pattern. No App Shell change.
