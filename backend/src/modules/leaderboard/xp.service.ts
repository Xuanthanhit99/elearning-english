...
export class XpService {
  ...
  async awardXp(input: AwardXpDto) {
    ...
    const finalXp = Math.max(-10000, Math.min(10000, input.baseXp + (input.bonusXp ?? 0)));
    await this.assertDailyLimit(input.userId, input.sourceType, Math.max(0, finalXp));
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      let entry: { id: string; groupId: string; periodXp: number } | null = null;
      if (activeSeason && !updatedProfile.optedOut) {
        entry = await this.ensureWeeklyEntry(tx, input.userId, profile.id, activeSeason.id);
    ...
    if (result.activeSeason && result.entry) {
      ...
      if (input.skill) {
        ...
        await this.redis.zincrby(skillRedisKey(input.skill, periodKey), finalXp, input.userId);
      }
  ...
  async reverseTransaction(transactionId: string, reason: string) {
    ...
    if (source.reversedAt) throw new ConflictException('Transaction already reversed');
  ...
  private async assertDailyLimit(userId: string, sourceType: XpSourceType, addingXp: number) {
  ...
  private async ensureWeeklyEntry(
    ...
    const profile = await tx.userXpProfile.findUniqueOrThrow({ where: { id: xpProfileId } });
