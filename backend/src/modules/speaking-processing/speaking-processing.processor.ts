...
export class SpeakingProcessingProcessor extends WorkerHost {
  private readonly logger = new Logger(SpeakingProcessingProcessor.name);
  ...
  async process(job: Job<SpeakingProcessingQueueData>) {
    if (job.name !== SPEAKING_PROCESSING_JOB.PROCESS_ANSWER) {
      throw new Error(`Unsupported Speaking job: ${job.name}`);
    }

    const { processingJobId, sessionId, answerId, userId } = job.data;
    ...
    try {
      await this.updateJob(processingJobId, {
        ...
        step: 'TRANSCRIBING',
        progress: 25,
        message: 'Đang chuyển giọng nói thành văn bản.',
        startedAt: new Date(),
      });

      const processing = await this.prisma.speakingProcessingJob.findUnique({
        where: {
          id: processingJobId,
        },
      });
      ...
      if (!processing) {
        throw new Error('SpeakingProcessingJob not found');
      }
      ...
      if (!session) {
        throw new Error('SpeakingSession not found');
      }

      const answer = await this.prisma.speakingAnswer.findUnique({
        where: {
          id: answerId,
        },
      });
      ...
      if (!answer) {
        throw new Error('SpeakingAnswer not found');
      }

      const transcription = await this.stt.transcribe({
        filepath: processing.audioPath,
        mimeType: processing.audioMimeType,
      });
      ...
      await this.updateJob(processingJobId, {
        step: 'AI_SCORING',
        progress: 55,
        message: 'AI đang phân tích phát âm, độ trôi chảy và ngữ pháp.',
      });
      ...
      const evaluation = await this.ai.evaluate({
        ...
        speechConfidence: transcription.confidence,
      });
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        const completed = await tx.speakingSession.updateMany({
          where: {
            id: sessionId,
            userId,
            status: {
              not: SpeakingSessionStatus.COMPLETED,
            },
          },
          data: {
        ...
        if (completed.count === 1 && session.lessonId) {
          const oldProgress = await tx.speakingLessonProgress.findUnique({
            where: {
              userId_lessonId: {
                userId,
      ...
      await this.updateJob(processingJobId, {
        ...
        step: 'COMPLETED',
        progress: 100,
        message: 'Đã hoàn thành phân tích bài nói.',
        missionUpdated,
        completedAt: new Date(),
      });
      ...
      await this.updateJob(processingJobId, {
        ...
        message: 'Không thể xử lý bài nói.',
      });
  ...
  onCompleted(job: Job) {
    this.logger.debug(`Speaking job completed: ${job.id}`);
  }

  @OnWorkerEvent('failed')
  onFailed(job: Job | undefined, error: Error) {
    this.logger.error(`Speaking job failed: ${job?.id}`, error.stack);
  }
}
