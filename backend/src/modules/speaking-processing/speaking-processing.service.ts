...
export class SpeakingProcessingService {
  ...
  async uploadAndQueue(input: {
    ...
    if (session.status === 'COMPLETED') {
      throw new BadRequestException(
        'Phiên luyện nói đã hoàn thành.',
      );
    }

    const stored =
      await this.audioStorage.save(
        input.file,
      );

    const answer =
      await this.prisma.speakingAnswer.create({
        data: {
          sessionId: session.id,
          question: input.dto.question,
          expectedText:
            input.dto.expectedText ??
            session.lesson?.expectedText ??
            '',
          transcript: '',
    ...
    const processingJob =
      await this.prisma.speakingProcessingJob.create({
        data: {
          userId: input.userId,
          sessionId: session.id,
          answerId: answer.id,
          ...
          progress: 10,
          message:
            'Đã tải audio lên. Đang chờ xử lý.',
          audioPath: stored.filepath,
          audioMimeType:
            stored.mimeType,
          duration:
            input.dto.duration ?? 0,
        },
      });
  ...
  async getStatus(
    ...
    const job =
      await this.prisma.speakingProcessingJob.findFirst({
        where: {
          userId,
          sessionId,
        },
    ...
    if (!job) {
      throw new NotFoundException(
        'Không tìm thấy tiến trình Speaking.',
      );
    }
  ...
  async getResult(
    ...
    if (session.status !== 'COMPLETED') {
      throw new BadRequestException(
        'Phiên luyện nói chưa xử lý xong.',
      );
    }
    ...
    if (!latestAnswer) {
      throw new NotFoundException(
        'Không tìm thấy kết quả Speaking.',
      );
    }
    ...
    return {
      ...
      scores: {
        ...
        pronunciation:
          latestAnswer.pronunciation,
        fluency:
          latestAnswer.fluency,
        grammar:
          latestAnswer.grammar,
        vocabulary:
          latestAnswer.vocabulary,
        confidence:
      ...
      answer: {
        ...
        expectedText:
          latestAnswer.expectedText,
        transcript:
          latestAnswer.transcript,
        audioUrl:
          latestAnswer.audioUrl,
        correctedText:
          latestAnswer.correctedText,
      },
      aiFeedback: {
        feedback:
          latestAnswer.feedback,
        suggestions:
          latestAnswer.suggestions,
        mistakes:
          processing?.mistakes ?? [],
        improvedVersion:
          processing?.improvedVersion ??
          latestAnswer.correctedText,
        nextPractice:
          processing?.nextPractice ??
          null,
      },
      missionUpdated:
        processing?.missionUpdated ??
        false,
    };
  }
}
