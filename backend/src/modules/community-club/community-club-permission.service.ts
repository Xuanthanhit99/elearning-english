...
export class CommunityClubPermissionService {
  ...
  async getClubManagementSummary(actorId: string, clubId: string) {
    ...
    const [club, members, pendingRequests, pendingInvites] =
      await Promise.all([
  ...
  async requestJoin(
    ...
    const existingMember =
      await this.prisma.communityClubMember.findUnique({
  ...
  async approveJoinRequest(
    ...
    const request =
      await this.prisma.communityClubJoinRequest.findFirst({
        where: {
          id: requestId,
          clubId,
          status: 'PENDING',
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const existing =
        await tx.communityClubMember.findUnique({
          where: {
            clubId_userId: {
              clubId,
  ...
  async rejectJoinRequest(
    ...
    const result =
      await this.prisma.communityClubJoinRequest.updateMany({
        where: {
          id: requestId,
          clubId,
          status: 'PENDING',
  ...
  async inviteMember(
    ...
    if (existingMember) {
      throw new ConflictException(
        'Người dùng đã là thành viên câu lạc bộ',
      );
    }
  ...
  async rejectInvite(userId: string, inviteId: string) {
    const result =
      await this.prisma.communityClubInvite.updateMany({
        where: {
          id: inviteId,
          inviteeId: userId,
          status: 'PENDING',
  ...
  async transferOwnership(
    ...
    if (actor.role !== 'OWNER') {
      throw new ForbiddenException(
        'Chỉ chủ câu lạc bộ mới được chuyển quyền',
      );
    }

    if (dto.newOwnerUserId === actorId) {
      throw new BadRequestException(
        'Người nhận quyền phải là thành viên khác',
      );
    }
  ...
  async updateMemberRole(
    ...
    const member =
      await this.prisma.communityClubMember.findFirst({
        where: {
          id: memberId,
          clubId,
        },
    ...
    return this.prisma.communityClubMember.update({
      where: { id: member.id },
      data: { role: dto.role },
      include: {
        user: { select: USER_SELECT },
  ...
  async kickMember(
    ...
    if (target.role === 'OWNER') {
      throw new ForbiddenException(
        'Không thể đuổi chủ câu lạc bộ',
      );
    }

    if (actor.role === 'ADMIN' && target.role === 'ADMIN') {
      throw new ForbiddenException(
        'Admin không thể đuổi Admin khác',
      );
    }

    if (target.userId === actorId) {
      throw new BadRequestException(
        'Hãy dùng chức năng rời câu lạc bộ',
      );
    }
  ...
  async leaveClub(userId: string, clubId: string) {
    ...
    if (member.role === 'OWNER') {
      const memberCount =
        await this.prisma.communityClubMember.count({
          where: { clubId },
        });
...
async deleteClub(actorId: string, clubId: string) {
  ...
  if (actor.role !== 'OWNER') {
    throw new ForbiddenException(
      'Chỉ chủ câu lạc bộ mới được xóa',
    );
  }
  ...
  private async getMembership(userId: string, clubId: string) {
    ...
    if (!membership) {
      throw new ForbiddenException(
        'Bạn không phải thành viên câu lạc bộ',
      );
    }
