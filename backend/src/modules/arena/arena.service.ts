...
`;
  ...
  async getRoom(userId: string, roomId: string) {
    ...
    return { ...room, isParticipant, serverTime: new Date().toISOString() };
  }
  ...
  async enterQueue(userId: string, dto: QueueArenaDto) {
    ...
    const opponent = await this.prisma.arenaQueue.findFirst({
      where: {
        userId: { not: userId },
        gameMode: dto.gameMode,
        skill: dto.skill,
    ...
    if (opponent) {
      ...
      await this.prisma.arenaQueue.deleteMany({
        where: { userId: { in: [userId, opponent.userId] } },
      });

      return { matched: true, room: await this.includeRoom(room.id) };
    }

    const range = this.getSearchRange(now, profile.mmr);
    const queue = await this.prisma.arenaQueue.upsert({
      where: { userId },
      update: {
        gameMode: dto.gameMode,
        skill: dto.skill,
  ...
  async submitAnswer(
    ...
    const answer = await this.prisma.arenaAnswer.upsert({
    ...
    const [questionCount, answerCount, freshParticipants, activeMatch] =
      await Promise.all([
        ...
        this.prisma.arenaMatch.findUnique({ where: { id: question.matchId } }),
      ]);
    if (
      activeMatch &&
      !activeMatch.finishedAt &&
      questionCount > 0 &&
      answerCount >= questionCount * freshParticipants.length
    ) {
      const teamAScore = freshParticipants
        .filter((item) => item.team === 'A')
        .reduce((sum, item) => sum + item.score, 0);
      const teamBScore = freshParticipants
        .filter((item) => item.team === 'B')
        .reduce((sum, item) => sum + item.score, 0);
      await this.finishMatch(room.hostId, roomId, {
        winnerTeam: teamBScore > teamAScore ? 'B' : 'A',
        result: {
          source: 'questions',
          teamAScore,
          teamBScore,
    ...
    return { answer, score, correct, wrong };
  }
  ...
  async finishMatch(userId: string, roomId: string, dto: FinishArenaMatchDto) {
    ...
    if (!room) throw new NotFoundException('Không tìm thấy phòng Arena');
    if (room.hostId !== userId)
      throw new ForbiddenException('Chỉ host được kết thúc trận demo');
    if (room.participants.length < 2)
      throw new BadRequestException(
    ...
    for (const participant of room.participants) {
      profiles.set(
        participant.userId,
        await this.getOrCreateProfile(participant.userId),
      );
    }
    ...
    const existingMatch = await this.prisma.arenaMatch.findFirst({
    ...
    for (const participant of room.participants) {
      ...
      const won = participant.team === dto.winnerTeam;
      ...
      await this.prisma.petProfile.updateMany({
        where: { userId: participant.userId },
        data: {
      ...
      const reward = await this.prisma.arenaRewardLog.create({
        data: {
          matchId: match.id,
