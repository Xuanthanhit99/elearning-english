...
export class PlacementTestService {
  ...
  async getSession(userId: string, sessionId: string) {
    ...
    return {
      ...
      currentQuestion: this.mapQuestion(current, currentIndex, session.questions),
      sections,
    ...
    await this.prisma.$transaction([
      ...
      this.prisma.placementTest.update({
        ...
        data: {
          ...
          correct: {
            increment:
              testQuestion.userAnswer === null && isCorrect ? 1 : 0,
          },
          score: {
            increment:
              testQuestion.userAnswer === null && isCorrect ? 1 : 0,
  ...
  private mapQuestion(
    ...
    const sectionIndex =
      skillQuestions.findIndex(
        (question) =>
          'id' in question &&
          (question as { id?: string }).id === item.id,
      ) + 1;
        ...
        return {
          ...
          text:
            typeof object.text === 'string'
              ...
              : String(object.value ?? ''),
          translation:
            typeof object.translation === 'string'
              ? object.translation
              : null,
        };
      }
